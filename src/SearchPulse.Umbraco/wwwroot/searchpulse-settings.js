...
class SearchPulseSettingsElement extends UmbElementMixin(HTMLElement) {
    ...
    render(settings) {
        ...
        const queueStatus = queueUsage < 75 ? "Healthy" : queueUsage < 95 ? "Busy" : "At capacity";
        ...
        this.innerHTML = [
            ...
            ".searchpulse-queue { display: grid; grid-template-columns: auto 1fr auto; gap: 12px; align-items: center; margin-top: 18px; } .searchpulse-queue strong { font-variant-numeric: tabular-nums; }",
            ...
            "<section class=\"searchpulse-box\"><h2>Collection health</h2><p class=\"searchpulse-muted\">Accepted events are stored in SearchPulse's durable local queue before they are added to reporting data.</p><div class=\"searchpulse-queue\"><strong>" + escapeHtml(queueStatus) + "</strong><div class=\"searchpulse-progress\" aria-label=\"Queue use " + queueUsage + "%\"><span style=\"width:" + queueUsage + "%\"></span></div><strong>" + formatNumber(settings.pendingEvents) + " / " + formatNumber(settings.maximumQueuedEvents) + " queued</strong></div></section>",
